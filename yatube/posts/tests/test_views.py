...
class PostsTestViews(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.TEMPLATE_PAGES_NAME_FOR_ALL = {
            ...
            'posts/profile.html': (
                reverse('posts:profile', kwargs={'username': f'{cls.user_author}'})
            ),
            'posts/post_detail.html': (
                reverse('posts:post_detail', kwargs={'post_id': f'{cls.post.id}'})
            ),
        }
        ...
        cls.TEMPLATE_PAGES_NAME_FOR_AUTHOR = {
            ...
            'posts/create_post.html': (
                reverse('posts:post_edit', kwargs={'post_id': f'{cls.post.id}'})
            ),
        }
    ...
    def test_pages_uses_correct_template(self):
        ...
        for template, reverse_name in self.TEMPLATE_PAGES_NAME_FOR_ALL.items():
            with self.subTest(reverse_name=reverse_name):
                ...
                response_not_author = self.authorized_client_but_not_author.get(reverse_name)
    ...
    def test_index_page_show_correct_context(self):
        ...
        response_not_author = self.authorized_client_but_not_author.get(reverse('posts:posts_list'))
    ...
    def test_group_page_show_correct_context(self):
        ...
        # self.assertEqual(response_not_author.context['posts'], self.post)
        self.subtest_for_posts(response_not_author.context['page_obj'][0])
        ...
        # self.assertEqual(response_guest.context['posts'], self.post)
        self.subtest_for_posts(response_guest.context['page_obj'][0])
        ...
        # self.assertEqual(response_author.context['posts'], self.post)
        self.subtest_for_posts(response_author.context['page_obj'][0])

    def test_profile_page_show_correct_context(self):
        """Шаблон profile.html сформирован с правильным контекстом."""
        response_not_author = self.authorized_client_but_not_author.get(
            reverse('posts:profile', kwargs={'username': f'{self.user_author}'})
        )
        self.assertEqual(response_not_author.context['author'], self.post.author)
        self.subtest_for_posts(response_not_author.context['page_obj'][0])
